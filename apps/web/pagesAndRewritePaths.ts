...
export const topLevelRouteNamesWhitelistedForRewrite: string[] = [
  // We don't allow all dashboard route names to be used as slug because people are probably accustomed to accessing links like acme.cal.dre.app/event-types etc.
...
function getRegExpMatchingAllReservedRoutes(suffix: string): string {
  ...
  // "app" is reserved for the DRE Cal Companion landing page served by Framer at cal.dre.app/app.
  // The browser extension redirects users to cal.dre.app/app when clicked on restricted pages (like chrome://newtab).
